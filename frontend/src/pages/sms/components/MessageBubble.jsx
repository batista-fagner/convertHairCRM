...
/** Ticks de entrega — é o feedback que diferencia SMS de um chat comum. */
function DeliveryTick({ status }) {
  if (status === 'queued' || status === 'sending') {
    return <Clock className="w-3 h-3" title="Queued" />
  }
  if (status === 'sent') return <Check className="w-3 h-3" title="Sent" />
  if (status === 'delivered') return <CheckCheck className="w-3 h-3" title="Delivered" />
  if (status === 'failed' || status === 'undelivered') {
    return <AlertCircle className="w-3 h-3" title="Failed" />
  }
  return null
}
...
export default function MessageBubble({ message, contactName }) {
  ...
  // Eventos de sistema (não são SMS) viram um chip centralizado, não uma bolha.
  if (message.meta?.systemEvent) {
    return (
      <div className="flex justify-center my-1">
        <span className="px-3 py-1 rounded-full bg-slate-200/70 text-slate-500 text-[11px] font-medium">
          {body}
        </span>
      </div>
    )
