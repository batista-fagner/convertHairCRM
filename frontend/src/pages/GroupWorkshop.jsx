    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-slate-800 truncate">{lead.name || 'Sem nome'}</p>
      ...
      {insights ? (
        <div className="space-y-2 mb-3">
          <div className="bg-amber-50 border border-amber-100 rounded-lg p-2.5">
            <p className="text-[10px] font-semibold text-amber-700 uppercase tracking-wide mb-0.5 flex items-center gap-1">
              <AlertCircle className="w-3 h-3" /> Dores mencionadas
            </p>
            <p className="text-xs text-slate-700">{insights.painPoints || 'Nenhuma dor específica mencionada na conversa.'}</p>
          </div>
          {insights.otherNotes && (
            <div className="bg-violet-50 border border-violet-100 rounded-lg p-2.5">
              <p className="text-[10px] font-semibold text-violet-700 uppercase tracking-wide mb-0.5 flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> Outras informações
              </p>
              <p className="text-xs text-slate-700">{insights.otherNotes}</p>
            </div>
          )}
        ...
        <p className="text-xs text-slate-400 italic mb-3">Conversa ainda não analisada.</p>
      )}

      <button
        onClick={() => onAnalyze(lead.id)}
        disabled={analyzing}
        className="w-full flex items-center justify-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-violet-200 text-violet-700 hover:bg-violet-50 disabled:opacity-50 transition"
      >
